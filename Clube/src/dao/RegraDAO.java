...
public class RegraDAO extends DAO<es2.atividade2.model.Regra> {
	...
	public Collection<Regra> select() {
		...
		try {
			...
			while (rs.next()) {
				...
				regra.add(new Regra(rs.getInt("id_regra"), rs
						.getString("nomeCartao"), rs.getString("horaInicial"),
						rs.getString("horaFinal"), getObjetoDiaDaSemana(rs
								.getInt("diasDaSemana_id")), rs
								.getString("numeroAcessoDia"), rs
								.getDouble("valor")));
			}
			rs.close();
		} catch (SQLException e) {
	...
	public Regra select(int id) {
		...
		try {
			...
			rs.next();
			regra = new Regra(rs.getInt("id_regra"),
					rs.getString("nomeCartao"), rs.getString("horaInicial"),
					rs.getString("horaFinal"),
					getObjetoDiaDaSemana(rs.getInt("diasDaSemana_id")),
					rs.getString("numeroAcessoDia"), rs.getDouble("valor"));
