...
public class DiasDaSemanaDAO extends DAO<es2.atividade2.model.DiasDaSemana> {
	...
	public Collection<DiasDaSemana> select() {
		...
		try {
			...
			while (rs.next()) {
				diasDaSemana.add(new DiasDaSemana(rs.getInt("id_diasDaSemana"),
						rs.getBoolean("monday"), rs.getBoolean("tuesday"), rs
								.getBoolean("wednesday"), rs
								.getBoolean("thursday"), rs
								.getBoolean("friday"), rs
								.getBoolean("saturday"), rs
								.getBoolean("sunday")));
			}
			rs.close();
		} catch (SQLException e) {
	...
	public DiasDaSemana select(int id) {
		...
		try {
			...
			rs.next();
			diasDaSemana = new DiasDaSemana(rs.getInt("id_diasDaSemana"),
