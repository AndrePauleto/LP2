...
public class SocioDAO extends DAO<es2.atividade2.model.Socio> {
	...
	public Collection<Socio> select() {
    	...
    	try{
    		...
    		while (rs.next()) {      			
    			socio.add(new Socio(rs.getInt("id_socio"), rs.getString("nome"), rs.getString("morada"), rs.getInt("idade"), getObjetoRegra(rs.getInt("cartao_id"))));
            }
    		rs.close();
    	}catch (SQLException e) {
	...
	public Socio select(int id) {
        ...
        try {
            ...
            rs.next();
            socio = new Socio(rs.getInt("id_socio"), rs.getString("nome"), rs.getString("morada"), rs.getInt("idade"), getObjetoRegra(rs.getInt("cartao_id")));
