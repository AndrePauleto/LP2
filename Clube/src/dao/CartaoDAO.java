...
public class CartaoDAO extends DAO<es2.atividade2.model.Cartao>{
	...
	public Collection<Cartao> select() {
    	...
    	try{
    		...
    		while (rs.next()) {      			
    			cartao.add(new Cartao(rs.getInt("id_cartao"), rs.getString("matricula"), (Regra) getObjetoRegra(rs.getInt("regra_id")), rs.getBoolean("acessando")));
            }
    		rs.close();
    	}catch (SQLException e) {
	...
	public Cartao select(int id) {
        ...
        try {
            ...
            rs.next();
            cartao = new Cartao(rs.getInt("id_cartao"), rs.getString("matricula"), (Regra) getObjetoRegra(rs.getInt("regra_id")), rs.getBoolean("acessando"));
